  ...
  if ($community===''){
    ...
    if(empty($codes)){
      ...
      if($idx>=0){
        ...
        $http_url = trim($_POST['http_url'] ?? '');
        if ($http_url !== '') {
          $newCommunity['http_url'] = $http_url;
        }
        $data[] = $newCommunity;
        write_json(GATES_JSON,$data);
  ...
  <div class="edit-modal-content">
    ...
    <div class="edit-modal-body" id="editModalBody">
      <form id="editCommunityForm" method="post">
        ...
        <div class="form-group" style="margin-bottom: 20px;">
          <label class="form-label">HTTP URL to Open Gate (optional)</label>
          <input type="url" class="field" name="http_url" id="editHttpUrl" placeholder="e.g., http://192.168.1.100/open">
        </div>
    ...
    <div class="edit-modal-body">
      <form id="addNewForm" method="post">
        ...
        <div class="form-group" style="margin-bottom: 20px;">
          ...
          <input type="hidden" name="coordinates" id="addNewCoordinates" value="">
        </div>

        <div class="form-group" style="margin-bottom: 20px;">
          <label class="form-label">HTTP URL to Open Gate (optional)</label>
          <input type="url" class="field" name="http_url" id="addNewHttpUrl" placeholder="e.g., http://192.168.1.100/open">
        </div>
...
function openEditModal(communityName) {
  ...
  // Load HTTP URL if available
  const httpUrlInput = document.getElementById('editHttpUrl');
  if (httpUrlInput) {
    httpUrlInput.value = community.http_url || '';
  }
...
document.addEventListener('click', (e) => {
  const editBtn = e.target.closest('.btn-edit-comm');
  if (editBtn) {
    const community = editBtn.getAttribute('data-community');
...
function openAddNewModal() {
  ...
  document.getElementById('addNewCityName').value = '';
  document.getElementById('addNewLatitude').value = '';
  document.getElementById('addNewLongitude').value = '';
  document.getElementById('addNewHttpUrl').value = '';
